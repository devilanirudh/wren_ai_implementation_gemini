...
// Replace lodash import with our own snakeCase function
...
// Simple custom snakeCase implementation
...
export class ProjectResolver {
  ...
  public async saveDataSource(
    ...
    args: {
      data?: DataSource;
      variables?: { data: DataSource };
    },
    ctx: IContext,
  ) {
    // Try to extract data from either args.data or args.variables.data
    let dataSource: DataSource | undefined = args.data;
    
    // If not found in args.data, try args.variables.data
    if (!dataSource && args.variables?.data) {
      dataSource = args.variables.data;
    }
    
    // Check if data is available in either location
    if (!dataSource) {
    ...
    if (!type || !properties) {
      throw new Error("Invalid input: type and properties are required");
    }
    
    // Currently only can create one project
    await this.resetCurrentProject(_root, args, ctx);

    // Extract displayName and connectionInfo, with fallbacks for missing properties
    const displayName = properties.displayName || "Unnamed Data Source";
    const connectionInfo = { ...properties };
    delete connectionInfo.displayName;
    ...
    try {
      ...
      // Continue despite dashboard init error
    }
    ...
    try {
      ...
      if (type === DataSourceName.DUCKDB) {
        const duckDbInfo = connectionInfo as DUCKDB_CONNECTION_INFO;
        await this.buildDuckDbEnvironment(ctx, {
          initSql: duckDbInfo.initSql || "",
          extensions: duckDbInfo.extensions || [],
          configurations: duckDbInfo.configurations || {},
        });
      } else {
        ...
        try {
          ...
          try {
            const version = await ctx.projectService.getProjectDataSourceVersion(project);
            await ctx.projectService.updateProject(project.id, {
              version,
            });
          } catch (versionErr) {
            logger.error(
              'Failed to get/update version',
              JSON.stringify(versionErr, null, 2),
            );
            // Continue despite version error
          }
          ...
          logger.error(
            ...
            JSON.stringify(tablesErr, null, 2),
          );
          throw tablesErr;
        }
      }
      ...
      try {
        ...
        logger.error(
          'Failed to cleanup project after error',
  ...
  public async updateDataSource(
    ...
    // Validate required properties
    if (!properties) {
      throw new Error("Invalid input: properties are required");
    }
    ...
    if (dataSourceType === DataSourceName.DUCKDB) {
      ...
      const duckDbInfo = toUpdateConnectionInfo as DUCKDB_CONNECTION_INFO;
      await this.buildDuckDbEnvironment(ctx, {
        initSql: duckDbInfo.initSql || "",
        extensions: duckDbInfo.extensions || [],
        configurations: duckDbInfo.configurations || {},

...
export class WrenAIAdaptor implements IWrenAIAdaptor {
  ...
  public async generateAskDetail(
    ...
    try {
      // Using sql-answers endpoint instead of ask-details
      const res = await axios.post(
        `${this.wrenAIBaseEndpoint}/v1/sql-answers`,
        {
          ...
          sql_data: {},
          thread_id: null,
          configurations: input.configurations || { language: "English" }
        },
      );
      
      // Make sure we have a valid response with query_id
      if (!res || !res.data || !res.data.query_id) {
        throw new Error("Invalid response from server: Missing query_id");
      }
      ...
      // Throw a custom error instead of the original one
      throw Errors.create(Errors.GeneralErrorCodes.INTERNAL_SERVER_ERROR, {
        customMessage: `Failed to generate SQL breakdown: ${getAIServiceError(err)}`,
        originalError: err,
      });
    }
  }
  ...
  public async getAskDetailResult(queryId: string): Promise<AskDetailResult> {
    try {
      ...
      // Return a failed result instead of throwing an error
      return {
        type: AskResultType.TEXT_TO_SQL,
        status: AskResultStatus.FAILED,
        error: {
          code: Errors.GeneralErrorCodes.INTERNAL_SERVER_ERROR,
          message: `Failed to get SQL breakdown: ${getAIServiceError(err)}`
        },
        response: {
          description: "Could not retrieve SQL breakdown",
          steps: []
        }
      };
    }
  ...
  private transformAskDetailResult(body: any): AskDetailResult {
    ...
    // When using sql-answers endpoint, the response structure is different
    // Provide default values for the expected AskDetailResult structure
    return {
      type: AskResultType.TEXT_TO_SQL,
      status: status as AskResultStatus,
      error,
      response: {
        description: "SQL result breakdown",
        steps: []
      },
    };
  }
  ...
  private transformStatusAndError(body: any): {
    ...
    // If body is undefined or null, provide a default status
    if (!body) {
      return {
        status: AskResultStatus.FAILED,
        error: {
    ...
    if (!status) {
      ...
      return {
        ...
        error: {
          code: Errors.GeneralErrorCodes.INTERNAL_SERVER_ERROR,
          message: `Unknown ask status: ${body?.status}`
        }
      };
    }
    ...
    const errorMessage = isShowAIServiceErrorMessage
      ? body?.error?.message
      : code
      ? Errors.errorMessages[code] || Errors.errorMessages[Errors.GeneralErrorCodes.INTERNAL_SERVER_ERROR]
      : null;

    // format error into WrenAIError that is used in graphql
    const formattedError = code
      ? {
          code: code as Errors.GeneralErrorCodes,
